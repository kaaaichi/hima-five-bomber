...
- [x] 5.2 タイマー機能（フロントエンド）
  ...
  - **実装完了**: Timerコンポーネントを実装し、全テスト（9個）がパス

- [x] 5.3 回答入力UI（フロントエンド）
  ...
  - **実装完了**: AnswerInputコンポーネントを実装し、全テスト（12個）がパス

- [x] 5.4 ゲームボード表示（フロントエンド）
  - GameBoardコンポーネントを実装
  - 問題文表示エリアを実装
  ...
  - **実装完了**: GameBoardコンポーネントを実装し、全テスト（11個）がパス。Timer、AnswerInput、AnswerGridを統合。プレイヤー番号表示とタイムアップ修正も完了

- [ ] 5.5 問題データの準備とS3アップロード
  - 問題JSONファイルの作成（questions.json）
  - 問題データの形式検証
  - S3バケットへのアップロード
  - バックエンドからの読み取りテスト
  - **Acceptance Criteria**:
    ```gherkin
    Given 問題データがJSON形式で準備されている
    When S3バケットにアップロードする
    Then バックエンドがS3から問題データを取得できる
    And 問題には正解データと表記ゆれが含まれる
    And 問題文のみがクライアントに送信される
    ```
  - _Requirements: R-002.9 (問題データ配信), R-010.3 (正解データの秘匿)_

- [ ] 5.6 ルーム待機画面のゲーム開始実装（フロントエンド）
  - ゲーム開始ボタンのWebSocket送信処理を実装
  - ゲームプレイ画面への遷移ロジックを実装
  - ホスト権限のバリデーションを実装
  - ゲーム開始時のローディング状態表示を実装
  - **Acceptance Criteria**:
    ```gherkin
    Given ホストがルーム待機画面にいる
    When ゲーム開始ボタンをクリックする
    Then WebSocketでstartGameメッセージが送信される
    And ゲームプレイ画面に遷移する
    And 問題文とタイマーが表示される

    Given ホスト以外のプレイヤーである
    When ルーム待機画面が表示される
    Then ゲーム開始ボタンが表示されない
    ```
  - _Requirements: R-001.5, R-002.1, R-002.8_

- [ ] 5.7 GamePlayページの完成（フロントエンド）
  - プレイヤー情報の取得と表示を実装
  - WebSocket経由での回答送信を実装
  - ゲーム状態とプレイヤー情報の統合を実装
  - 回答結果のフィードバック表示を実装
  - **Acceptance Criteria**:
    ```gherkin
    Given ゲームが開始されている
    When GamePlayページが表示される
    Then プレイヤー情報（名前、順番）が表示される
    And 自分が何番目のプレイヤーか表示される
    And 現在の回答順がハイライトされる
    ...
    And 次の回答順に進む
    ```
  - _Requirements: R-002.3, R-002.4, R-002.5, R-004.1_
  - **実装状況**: 基本実装完了。プレイヤー情報の統合が残タスク（players配列がTODO）

- [ ] 5.8 エンドツーエンド統合とデプロイ確認
  - **事前条件（IaCリポジトリで完了必須）**:
    - WebSocket APIのデプロイ
    - GameSessionsテーブルのデプロイ
    - S3問題データバケットのデプロイ
  - フロントエンド環境変数の設定（VITE_WS_URL）
  - ルーム作成→参加→ゲーム開始→回答送信の一連の流れをテスト
  - WebSocketメッセージフローの確認
  - エラーハンドリングの確認
  - **Acceptance Criteria**:
    ```gherkin
    Given IaCでWebSocket API、GameSessions、S3がデプロイされている
    When CloudFront URLでアプリを開く
    Then ルーム作成からゲームプレイまでの流れが動作する
    And WebSocketでリアルタイム同期される
    And 回答が正しく判定される
    And タイムアップ処理が動作する
    ```
  - _Requirements: R-001.1-1.7, R-002.1-2.13, R-004.1-4.2_
...
- [x] 6.4 回答結果の反映（フロントエンド）
  ...
  - **実装完了**:
    - ✅ AnswerFeedbackコンポーネント（正解マーク・不正解フィードバック表示）
    - ✅ AnswerSlotコンポーネント（個別の回答枠、現在の回答順ハイライト、自分の枠強調）
    - ✅ AnswerGridコンポーネント（5つの回答枠管理、フォーカス移動）
    - ✅ useGameStateでのanswerResult処理ロジック（既存実装）
    - ✅ 全テストがパス（AnswerFeedback: 6テスト、AnswerSlot: 6テスト、AnswerGrid: 6テスト）
